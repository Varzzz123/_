...
const TEBAK_ANGKA_MAX = 100;

const TRIVIA = [
    { soal: "Ibu kota Indonesia adalah?", jawab: "jakarta" },
    { soal: "Planet terbesar di tata surya adalah?", jawab: "jupiter" },
    { soal: "Berapa jumlah provinsi di Indonesia (2024)?", jawab: "38" },
    { soal: "Siapa penemu telepon?", jawab: "alexander graham bell" },
    { soal: "Negara terluas di dunia adalah?", jawab: "rusia" },
    { soal: "Gunung tertinggi di dunia?", jawab: "everest" },
    { soal: "Warna bendera Indonesia terdiri dari berapa warna?", jawab: "2" },
    { soal: "Apa bahasa resmi Brasil?", jawab: "portugis" },
    { soal: "Berapa sisi kubus?", jawab: "6" },
    { soal: "Di film Avengers, siapa yang menggunakan perisai vibranium?", jawab: "captain america" },
];
...
async function handleGames(ctx) {
    ...
    if (activeSession && !body.startsWith(p)) {
...
async function handleAnswer(ctx, session, sessionKey) {
    ...
    if (session.type === "tebakkata" || session.type === "trivia" || session.type === "tebakemoji") {
        ...
        if (jawab === session.jawab) {
            ...
            if (session.attempts >= 3) {
                sessions.delete(sessionKey);
                return reply(`❌ Salah 3x! Jawaban yang benar: *${session.jawab}*`);
            }
    ...
    if (session.type === "tebakangka") {
        ...
        } else if (tebak < session.jawab) {
            return reply(`📈 Terlalu kecil! Coba lagi.`);
        } else {
            return reply(`📉 Terlalu besar! Coba lagi.`);
        }
    }
    ...
    if (session.type === "matematika") {
        ...
        if (jawab === session.jawab) {
            ...
            sessions.delete(sessionKey);
            return reply(`❌ Salah! Jawaban yang benar: *${session.jawab}*`);
        }
    }
    ...
    if (session.type === "hangman") {
        ...
        if (!session.word.includes(huruf)) {
            session.wrong++;
        }
        ...
        if (solved) {
            ...
            saveDB(db);
            return reply(`✅ *Kamu berhasil!* Kata: *${session.word}* +20 poin`);
        }

        if (session.wrong >= 6) {
            sessions.delete(sessionKey);
            return reply(`💀 Game over! Kata yang benar: *${session.word}*`);
        }
        ...
        return sock.sendMessage(from, {
            text:
                `${hangmanArt[session.wrong]} Nyawa: ${6 - session.wrong}/6\n` +
                `Kata: ${display}\n` +
...
async function startTebakKata(ctx, sessionKey) {
    ...
    const soalObj = randomItem(TEBAK_KATA);
    sessions.set(sessionKey, { type: "tebakkata", jawab: soalObj.jawab, attempts: 0 });
...
async function startTebakAngka(ctx, sessionKey) {
    const { sock, msg, from } = ctx;
    const angka = Math.floor(Math.random() * TEBAK_ANGKA_MAX) + 1;
    sessions.set(sessionKey, { type: "tebakangka", jawab: angka });
    setTimeout(() => sessions.delete(sessionKey), 120000);
    await sock.sendMessage(from, {
        text: `🔢 *TEBAK ANGKA*\n\nAku punya angka antara 1 - ${TEBAK_ANGKA_MAX}\nCoba tebak! (120 detik)`,
    }, { quoted: msg });
}

async function startTrivia(ctx, sessionKey) {
    const { sock, msg, from } = ctx;
    const soalObj = randomItem(TRIVIA);
    sessions.set(sessionKey, { type: "trivia", jawab: soalObj.jawab, attempts: 0 });
    setTimeout(() => sessions.delete(sessionKey), 60000);
    await sock.sendMessage(from, {
        text: `📚 *TRIVIA*\n\n${soalObj.soal}\n\n_Ketik jawabanmu! (60 detik)_`,
    }, { quoted: msg });
...
async function handleSuit(ctx) {
    ...
    if (pilihan === botPilih) {
        hasil = "🤝 *Seri!*";
    } else if (SUIT_WIN[pilihan] === botPilih) {
        hasil = `🎉 *${senderName} menang!*`;
    } else {
        hasil = `🤖 *Bot menang!*`;
    }
...
async function startTebakEmoji(ctx, sessionKey) {
    const { sock, msg, from } = ctx;
    const soalObj = randomItem(TEBAK_EMOJI_LIST);
    sessions.set(sessionKey, { type: "tebakemoji", jawab: soalObj.jawab, attempts: 0 });
    setTimeout(() => sessions.delete(sessionKey), 60000);

    await sock.sendMessage(from, {
        text: `🎭 *TEBAK EMOJI*\n\n${soalObj.soal}\n\nApa yang dimaksud emoji di atas? (60 detik)`,
    }, { quoted: msg });
}
